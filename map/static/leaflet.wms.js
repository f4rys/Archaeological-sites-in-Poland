...
wms.Source = L.Layer.extend({
    ...
    'showFeatureInfo': async function(latlng, info) {
        ...
        if (info.match(/(?<=\>)(.*?)(?=<\/BODY>)/)[1].trim().length > 0) {
            // if there is any info in response
            info = info.match(/(PL\.\S+)/)[1]
            //import info from database based on INSPIRE_ID value
            info = this.importDatabase(info)
            info = await info;
    ...
    'importDatabase': function(info){
        var url = '/api/get_excavation_data/' + info + '/';
          ...
          xhr.onreadystatechange = function() {
            if (xhr.readyState === 4) {
              if (xhr.status === 200) {
                ...
                var table = '<TABLE><TR><TD class="featureTh">Funkcja:</TD><TD></TD><TD class="featureTd">'
                table += data['Funkcja']
                table += '</TD></TR><TR><TD class="featureTh">Chronologia:</TD><TD></TD><TD class="featureTd">'
                table += data['Chronologia']
                table += '</TD></TR><TR><TD class="featureTh">Miejscowość:</TD><TD></TD><TD class="featureTd">'
                table += data['Miejscowość']
                table += '</TD></TR><TR><TD class="featureTh"></TD><TD></TD></TR></TABLE><DIV><A href="'
                table += data['Link']
                table += '">Więcej informacji</A></DIV>'
                
                // Resolve the Promise with the HTML table element
                resolve(table);
              } else {
                // Reject the Promise with an error message on HTTP error
